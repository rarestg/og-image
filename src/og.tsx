...
export default app.get('/', async (c) => {
	...
	const textParts = [mainText, description];
	...
	// ********************** Google Fonts ********************** //
	// const font = await googleFont(`${mainText ?? ''}${description ?? ''}${footerText ?? ''}`, 'Noto Sans JP', 900, 'normal');
		...
		return (
			...
			>
				<span tw="my-3 px-5 flex items-center">
					<img
						src={`${new URL(c.req.url).origin}/images/cf-logo-v-rgb.png`}
						height="60"
						alt="Cloudflare"
					/>
				</span>
